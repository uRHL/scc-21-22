...
cewl -m 6  -d 1 --lowercase --convert-umlauts https://en.wikipedia.org/wiki/Corinna_zu_Sayn-Wittgenstein-Sayn >> $OUTPUT_FILE

aux=`wc -l < $OUTPUT_FILE`

echo "Word count from Wikipedia page (English version): $aux"

cewl -m 6  -d 1 --lowercase --convert-umlauts https://es.wikipedia.org/wiki/Corinna_Larsen >> $OUTPUT_FILE
